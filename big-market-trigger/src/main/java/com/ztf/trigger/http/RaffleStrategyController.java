...
import com.ztf.trigger.api.dto.*;
...
public class RaffleStrategyController implements IRaffleStrategyService {
    ...
    public Response<List<RaffleAwardListResponseDTO>> queryRaffleAwardList(@RequestBody RaffleAwardListRequestDTO request) {
        try {
            log.info("查询抽奖奖品列表配开始 strategyId：{}", request.getUserId(), request.getActivityId());
            //1.参数校验
            ...
            /**
             * 这里获取到的treeIds是什么
             * 是那些rule_lock、rule_weight等名字，可以确定对应的规则树节点
             * 通过一个treeId就可以确定一个treeNode数据，这个数据中有对应的ruleValue属性，表示对应的规则值
             * 比如rule_lock_1是一个treeId，对应有ruleValue为2表示抽奖2次之后可以解锁这个奖品
             */
